  ...
  tools: [
    {
      name: "create_task",
      description:
        "Create a new pickup-and-deliver task on the Meatspace marketplace. " +
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    if (name === "create_task") {
      const deadline =
        Math.floor(Date.now() / 1000) + (args!.deadlineMinutes as number) * 60;
    ...
    if (name === "get_task_status") {
      ...
      const workerShort =
        data.worker === "0x0000000000000000000000000000000000000000"
          ? "None"
